...
export const FoodCard: React.FC<Props> = ({ item }) => {
  ...
  return (
    <div
      className={`group bg-white rounded-2xl border transition-all overflow-hidden flex flex-col hover:shadow-md ${
        available
    ...
    >
      <Link href={`/menu/${itemId}`} className="block relative aspect-[4/3] w-full overflow-hidden bg-slate-100">
        ...
        <div className="absolute top-2.5 left-2.5 flex items-center gap-1.5 flex-wrap">
          {isVegan ? (
            <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-600 text-white shadow-sm">
              Vegan
            </span>
          ) : isJain ? (
            ...
            <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-700 text-white shadow-sm">
              Veg
            </span>
          ) : (
            <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-rose-700 text-white shadow-sm">
              Non-Veg
            </span>
          )}

          {isSpicy && (
            <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-orange-600 text-white shadow-sm">
              🌶️ Spicy
            </span>
          )}
        </div>
        ...
        <div className="absolute top-2.5 right-2.5">
          {available ? (
            <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-white/95 text-emerald-700 shadow-sm border border-emerald-200 flex items-center gap-1">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
              Available
            </span>
          ) : (
            <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-rose-50 text-rose-700 shadow-sm border border-rose-200">
              Unavailable
            </span>
          )}
        </div>
      ...
      <div className="p-4 flex flex-col flex-1 justify-between gap-3">
        <div>
          ...
          <div className="flex items-center gap-2 mt-1 text-xs text-slate-500">
            <span className="font-extrabold text-[#0C3B25] text-sm">₹{price}</span>
            <span>•</span>
        ...
        <div className="bg-[#F8FAF7] border border-slate-100 rounded-xl p-2 text-center">
        ...
        <div className="flex items-center gap-2 pt-1">
          <Link
            href={`/menu/${itemId}`}
            className="flex-1 text-center py-2 text-xs font-semibold text-gray-700 hover:text-emerald-700 bg-gray-50 hover:bg-emerald-50/50 rounded-xl border border-gray-200 transition-colors"
          >
            Details
          </Link>
          <button
            ...
            disabled={!available}
            className="flex items-center justify-center gap-1 px-3.5 py-2 bg-[#059669] hover:bg-[#047857] text-white text-xs font-bold rounded-xl shadow-sm hover:shadow transition-all disabled:opacity-40 disabled:pointer-events-none"
